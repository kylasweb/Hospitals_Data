...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        response = requests.get(
            f'https://data.gov.in/api/datastore/resource.json?resource_id=e16c75b6-7ee6-4ade-8e1f-2cd3043ff4c9&api-key={settings.API_KEY}'
        )
        ...
        for i in range(int(offset)):
            response = requests.get(
                f'https://data.gov.in/api/datastore/resource.json?resource_id=e16c75b6-7ee6-4ade-8e1f-2cd3043ff4c9&api-key={settings.API_KEY}&offset={str(i)}'
            )
        ...
        for each_list in hospitals_list:
            parse_list.extend(iter(each_list))
